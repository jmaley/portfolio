...
// (private) render:
template <typename T, typename P, typename C>
void kdtree_t<T, P, C>::render(node_t* &t)
{
	if (t != NULL)
	{
		render(t->left);
		...
		{
			glBegin(GL_LINES);
			glVertex2f((*t->data)[0], (*t->data)[1]);
			glVertex2f((*t->data)[0], t->min[1]);
			//glEnd();

			//glBegin(GL_LINES);
			glVertex2f((*t->data)[0], (*t->data)[1]);
			glVertex2f((*t->data)[0], t->max[1]);
			glEnd();
		}
		else
		{
			glBegin(GL_LINES);
			glVertex2f((*t->data)[0], (*t->data)[1]);
			glVertex2f(t->min[0], (*t->data)[1]);

			glVertex2f((*t->data)[0], (*t->data)[1]);
			glVertex2f(t->max[0], (*t->data)[1]);
			glEnd();
		}
